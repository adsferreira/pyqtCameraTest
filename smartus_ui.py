...
from PyQt5.QtWidgets import QMainWindow, QLabel, QApplication, QWidget, QVBoxLayout, QPushButton
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.__counter = 0
        self.__prefix = "/home/adriano/PycharmProjects/pyqtCameraTest/images/"
        self.__widget = QWidget()
        self.__depth_label = QLabel()
        self.__rgb_label = QLabel()
        self.__qv_box_layout = QVBoxLayout()
        self.__button = QPushButton('Save Images', self)
        self.__button.setToolTip('Save color and depth images.')
        self.__button.clicked.connect(self.save_images)
        self.__qv_box_layout.addWidget(self.__depth_label)
        self.__qv_box_layout.addWidget(self.__rgb_label)
        self.__qv_box_layout.addWidget(self.__button)
    ...
    def save_images(self):
        depth_pixmap = self.__depth_label.pixmap()
        color_pixmap = self.__rgb_label.pixmap()
        depth_pixmap.save(self.__prefix + "animal_0_img_" + str(self.__counter) + "_depth.png", "PNG")
        color_pixmap.save(self.__prefix + "animal_0_img_" + str(self.__counter) + "_color.png", "PNG")
        self.__counter += 1
